...
sudo tee /etc/systemd/system.conf.d/99-omybuntu-nofile.conf >/dev/null <<'EOF'
[Manager]
DefaultLimitNOFILE=65536:524288
EOF

sudo cp /etc/systemd/system.conf.d/99-omybuntu-nofile.conf \
        /etc/systemd/user.conf.d/99-omybuntu-nofile.conf
