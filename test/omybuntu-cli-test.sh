...
output=$("$CLI" --help)
assert_output_contains "main help renders" "$output" "Omybuntu command center"
...
output=$("$CLI" commands)
assert_output_contains "commands lists documented commands" "$output" "omybuntu theme set <theme-name>"
...
"$CLI" commands --all --json | jq -e '.commands[] | select(.route == "omybuntu hyprland window gaps toggle" and .summary != "undocumented")' >/dev/null
pass "fallback commands are inferred and documented"

"$CLI" commands --all --json | jq -e '.commands[] | select(.route == "omybuntu dev benchmark")' >/dev/null
...
assert_output_contains "install group includes browser route" "$output" "omybuntu install browser"

output=$("$CLI" install)
assert_output_contains "bare group renders help instead of picker" "$output" "Install commands"
assert_output_contains "bare group includes browser route" "$output" "omybuntu install browser"

output=$("$CLI" toggle)
assert_output_contains "bare root command with children renders help" "$output" "Toggle commands"
assert_output_contains "bare toggle help includes child route" "$output" "omybuntu toggle waybar"

output=$("$CLI" pkg --help)
assert_output_contains "package group includes pkg add fallback route" "$output" "omybuntu pkg add <packages...>"

output=$("$CLI" restart --help)
assert_output_contains "restart group includes inferred commands" "$output" "omybuntu restart btop"
assert_output_contains "restart group includes all restart commands" "$output" "omybuntu restart wifi"

output=$("$CLI" hw --help)
assert_output_contains "hardware group help renders" "$output" "omybuntu hw asus rog"
assert_output_contains "hardware group includes touchpad" "$output" "omybuntu hw touchpad"

output=$("$CLI" hw asus)
assert_output_contains "partial hardware prefix renders matching commands" "$output" "omybuntu hw asus rog"
assert_output_contains "partial hardware prefix includes nested match" "$output" "omybuntu hw asus zenbook ux5406aa"

output=$("$CLI" menu --help)
assert_output_contains "menu group includes share fallback route" "$output" "omybuntu menu share"

output=$("$CLI" share)
assert_output_contains "bare required-arg alias renders CLI help" "$output" "Usage:"
assert_output_contains "bare share help uses canonical route" "$output" "omybuntu share <clipboard|file|folder> [path...]"
...
for command in commands:
  ...
  stem = binary.removeprefix('omybuntu-')
  group = stem.split('-', 1)[0]
  filename_route = 'omybuntu ' + stem.replace('-', ' ')
...
assert_output_contains "theme set help names binary" "$output" "omybuntu-theme-set"

output=$(timeout 5 "$CLI" update --help)
assert_output_contains "mutating command help does not execute target" "$output" "omybuntu-update"
assert_output_contains "root command help shows related child commands" "$output" "omybuntu update perform"

output=$("$CLI" screenshot --help)
assert_output_contains "root alias resolves to command help" "$output" "omybuntu-capture-screenshot"

"$CLI" commands --json | jq -e '.commands[] | select(.binary == "omybuntu-capture-screenshot") | .aliases | index("omybuntu screenshot")' >/dev/null
pass "aliases are included in JSON metadata"

output=$("$CLI" pkg add --help)
assert_output_contains "pkg add help resolves" "$output" "omybuntu-pkg-add"
assert_output_contains "pkg add help shows direct route" "$output" "omybuntu pkg add <packages...>"

output=$("$CLI" system reboot --help)
assert_output_contains "system command help is safe" "$output" "omybuntu-system-reboot"

output=$("$CLI" dev benchmark --repeat=1)
assert_output_contains "benchmark command runs" "$output" "Omybuntu CLI benchmark"
...
while IFS= read -r binary_path; do
  ...
  grep -q '^# omybuntu:summary=' <<<"$header" || fail "metadata summary is present: $binary_path"
  ! grep -q '^# omybuntu:binary=' <<<"$header" || fail "metadata does not repeat inferred binary: $binary_path"
  ! grep -q '^# omybuntu:args=$' <<<"$header" || fail "metadata does not include empty args: $binary_path"
  ! grep -Eq '^# omybuntu:(legacy|usage|visibility|mutates|interactive)=' <<<"$header" || fail "metadata avoids removed fields: $binary_path"
  ! grep -Eq '^# omybuntu:requires-sudo=false$' <<<"$header" || fail "metadata omits false booleans: $binary_path"
done < <(find "$ROOT/bin" -maxdepth 1 -type f -executable -name 'omybuntu-*' | sort)
pass "all executable bins have slim self-documenting metadata"

TMPDIR=$(mktemp -d)
ln -s "$CLI" "$TMPDIR/omybuntu"

{
  printf '#!/bin/bash\n\n'
  printf '# ordinary comments are fine\n'
  printf '# omybuntu:this malformed line should be ignored\n'
  printf '# omybuntu:group=weird\n'
  printf '# omybuntu:name=test\n'
  printf '# omybuntu:summary=Survives malformed metadata comments\n'
  printf '# omybuntu:made-up=value\n'
  printf 'echo weird-ok\n'
} >"$TMPDIR/omybuntu-weird-test"
chmod +x "$TMPDIR/omybuntu-weird-test"

{
  printf '#!/bin/bash\n\n'
  printf '# a partial metadata header should not destroy fallback routing\n'
  printf '# omybuntu:summary=Partial metadata keeps inferred route\n'
  printf '# omybuntu:made-up=value\n'
  printf 'echo partial-ok\n'
} >"$TMPDIR/omybuntu-partial-meta-test"
chmod +x "$TMPDIR/omybuntu-partial-meta-test"

{
  printf '#!/bin/bash\n\n'
  printf 'echo body-metadata-ok\n'
  printf '# omybuntu:group=wrong\n'
  printf '# omybuntu:name=wrong\n'
} >"$TMPDIR/omybuntu-body-metadata-test"
chmod +x "$TMPDIR/omybuntu-body-metadata-test"

"$TMPDIR/omybuntu" commands --all --json | jq -e '.commands[] | select(.route == "omybuntu weird test" and .summary == "Survives malformed metadata comments")' >/dev/null
pass "unknown metadata values are non-fatal"

"$TMPDIR/omybuntu" commands --all --json | jq -e '.commands[] | select(.route == "omybuntu partial meta test" and .summary == "Partial metadata keeps inferred route")' >/dev/null
pass "partial metadata keeps inferred fallback route"

"$TMPDIR/omybuntu" commands --all --json | jq -e '.commands[] | select(.route == "omybuntu body metadata test" and .summary == "Run the body metadata test command")' >/dev/null
pass "metadata-looking comments after script body are ignored"

output=$("$TMPDIR/omybuntu" weird test)
assert_output_contains "temporary metadata command dispatches" "$output" "weird-ok"

output=$("$TMPDIR/omybuntu" partial meta test)
