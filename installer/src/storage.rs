    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::AlongsideWindows {
                ...
                .field("boot_partition_number", boot_partition_number)
...
pub fn validate_alongside_plan(plan: &StoragePlan) -> Result<(), String> {
    ...
    let valid = candidates.iter().any(|candidate| {
        candidate.disk.path == *disk
            ...
            && candidate.boot_partition_number == *boot_partition_number
...
fn discover_disk_candidates(
    ...
    let used_numbers: BTreeSet<u32> = children
        .iter()
        .filter_map(|child| {
        ...
        .collect();
    let mut free_numbers = (1..=128).filter(|number| !used_numbers.contains(number));
    let boot_partition_number = free_numbers.next().unwrap_or(0);
    let root_partition_number = free_numbers.next().unwrap_or(0);
    if boot_partition_number == 0 || root_partition_number == 0 {
        return Ok(vec![]);
    }
    ...
    Ok(eligible_free_regions(regions)
        .into_iter()
...
fn esp_contains_windows(partition: &str, existing_mountpoints: &[String]) -> Result<bool, String> {
    if let Some(mountpoint) = existing_mountpoints.iter().find(|path| !path.is_empty()) {
        return Ok(Path::new(mountpoint)
            .join("EFI/Microsoft/Boot/bootmgfw.efi")
            .is_file());
    }

    let mount_dir = PathBuf::from(format!("/run/omyvoid-installer/esp-{}", std::process::id()));
    ...
    fn requires_at_least_64_gib_of_contiguous_free_space() {
        let regions = vec![
            FreeRegion {
                start_sector: 1,
                end_sector: 2,
                size_bytes: MIN_ALONGSIDE_BYTES - 1,
            },
            FreeRegion {
                start_sector: 3,
                end_sector: 4,
                size_bytes: MIN_ALONGSIDE_BYTES,
            },
        ];
