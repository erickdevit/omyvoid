...
# Users can still opt-in to Cloudflare DNS using: omybuntu-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-omybuntu-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-omybuntu-dns.network
  ...
  echo "To use Cloudflare DNS, run: omybuntu-setup-dns Cloudflare"
fi