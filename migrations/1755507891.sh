...
echo "Migrating to use omybuntu-launch-webapp and omybuntu-launch-browser"
for desktop_file in ~/.local/share/applications/*.desktop; do
  if grep -q 'Exec=chromium --new-window --ozone-platform=wayland --app=' "$desktop_file"; then
    ...
    if [[ -n $url ]]; then
      sed -i "s|^Exec=.*|Exec=omybuntu-launch-webapp \"$url\"|" "$desktop_file"
    fi
  fi
done
...
if [[ -f $HYPR_BINDINGS_FILE ]]; then
  sed -i 's/\$browser =.*chromium.*$/\$browser = omybuntu-launch-browser/' "$HYPR_BINDINGS_FILE"
  sed -i 's/\$webapp="/omybuntu-launch-webapp "/g' "$HYPR_BINDINGS_FILE"
