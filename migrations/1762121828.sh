...
omybuntu-pkg-add xdg-terminal-exec
...
if command -v alacritty > /dev/null 2>&1; then
  cp "$OMYBUNTU_PATH/applications/Alacritty.desktop" ~/.local/share/applications/
fi
...
if [[ -f $waybar_config ]]; then
  sed -i 's|"on-click-right": "omybuntu-launch-terminal"|"on-click-right": "xdg-terminal-exec"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL -e btop"|"on-click": "xdg-terminal-exec btop"|' "$waybar_config"
  sed -i 's|"on-click": "\$TERMINAL --class=Wiremix -e wiremix"|"on-click": "xdg-terminal-exec --app-id=com.omybuntu.Wiremix -e wiremix"|' "$waybar_config"
  omybuntu-state set restart-waybar-required
fi

# Update hyprland window rules to use DNS-format class names
system_conf=~/.config/hypr/apps/system.conf
if [[ -f $system_conf ]]; then
  if grep -q 'class:(.*|Impala|' "$system_conf" || grep -q 'class:(.*|Wiremix|' "$system_conf" || grep -q '|Omybuntu|' "$system_conf"; then
    sed -i 's/\bImpala\b/com.omybuntu.Impala/g; s/\bWiremix\b/com.omybuntu.Wiremix/g; s/|Omybuntu|/|com.omybuntu.Omybuntu|/g' "$system_conf"
  fi
fi
