...
OBSIDIAN_DEFAULT_FLAGS_FILE="$OMYBUNTU_PATH/config/obsidian/user-flags.conf"
...
if [[ ! -f $OBSIDIAN_FLAGS_FILE ]]; then
  ...
  if [[ -n $missing_flags ]]; then
    {
      echo
      echo "# Added by Omybuntu migration 1776434586: Obsidian launch flags moved from Hyprland binding."
      printf "%s\n" "$missing_flags"
    } >>"$OBSIDIAN_FLAGS_FILE"
  fi
fi
