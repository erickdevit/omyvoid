...
// --- CONFIGURATION ---
const float CURSOR_SLANT = 0.25; // Skew angle factor for diagonal slant (0.0 = standard, 0.25 = modern slant)
const float CURSOR_OPACITY = 0.6; // Opacity of the custom cursor (0.0 to 1.0)
const float TRAIL_OPACITY = 0.45; // Opacity of the trail (0.0 to 1.0)
...
vec2 skew(vec2 p, float slant) {
    return vec2(p.x - p.y * slant, p.y);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    // Apply skew mapping to coordinate space and centers for slanted look
    vec2 vu_skewed = skew(vu, CURSOR_SLANT);
    vec2 centerCC_skewed = skew(centerCC, CURSOR_SLANT);
    vec2 centerCP_skewed = skew(centerCP, CURSOR_SLANT);

    vec2 delta = centerCP - centerCC;
    float lineLength = length(delta);

    // The current cursor shape evaluated in the skewed space (becomes slanted)
    float sdfCurrentCursor = getSdfRectangle(vu_skewed, centerCC_skewed, currentCursor.zw * 0.5);
    ...
    if (lineLength > minDist) {
        ...
        // -- Making the parallelogram sdf (diagonal move) in skewed space --

        // animate the TOP-LEFT corners (skewed)
        ...
        vec2 head_pos_tl_skewed = skew(head_pos_tl, CURSOR_SLANT);
        vec2 tail_pos_tl_skewed = skew(tail_pos_tl, CURSOR_SLANT);
        ...
        // v0, v1 : "front" of the trail (head) in skewed space
        vec2 v0 = vec2(head_pos_tl_skewed.x + currentCursor.z * isTopRightLeading, head_pos_tl_skewed.y - currentCursor.w);
        vec2 v1 = vec2(head_pos_tl_skewed.x + currentCursor.z * isBottomLeftLeading, head_pos_tl_skewed.y);
        
        // v2, v3: "back" of the trail (tail) in skewed space
        vec2 v2 = vec2(tail_pos_tl_skewed.x + currentCursor.z * isBottomLeftLeading, tail_pos_tl_skewed.y);
        vec2 v3 = vec2(tail_pos_tl_skewed.x + currentCursor.z * isTopRightLeading, tail_pos_tl_skewed.y - previousCursor.w);

        float sdfTrail_diag = getSdfParallelogram(vu_skewed, v0, v1, v2, v3);

        // -- Making the rectangle sdf (straight move) in skewed space --

        vec2 head_center_skewed = mix(centerCP_skewed, centerCC_skewed, head_eased);
        vec2 tail_center_skewed = mix(centerCP_skewed, centerCC_skewed, tail_eased);

        vec2 min_center = min(head_center_skewed, tail_center_skewed);
        vec2 max_center = max(head_center_skewed, tail_center_skewed);
        
        vec2 box_size = (max_center - min_center) + currentCursor.zw;
        vec2 box_center = (min_center + max_center) * 0.5;

        float sdfTrail_rect = getSdfRectangle(vu_skewed, box_center, box_size * 0.5);
    ...
    // Punch a hole in the trail where the current cursor is, so we can draw the custom cursor cleanly on top of the text
    newColor = mix(newColor, fragColor, step(sdfCurrentCursor, 0.));

    // Draw the current custom slanted cursor (always visible, both static and moving)
    float cursorAlpha = antialising(sdfCurrentCursor);
    vec4 cursorColor = vec4(linearCursorColor, CURSOR_OPACITY);
    newColor = mix(newColor, cursorColor, cursorAlpha);
