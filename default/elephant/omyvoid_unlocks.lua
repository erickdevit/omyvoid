--
-- Dynamic Omyvoid Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the omyvoid-shipped Plymouth via
-- omyvoid-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs omyvoid-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "omyvoidunlocks"
NamePretty = "Omyvoid Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/omyvoid/themes"
  local omyvoid_path = os.getenv("OMYVOID_PATH") or ""
  local default_themes_dir = omyvoid_path .. "/themes"
  local default_preview = omyvoid_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "omyvoid-launch-floating-terminal-with-presentation "
                .. shell_escape("omyvoid-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "omyvoid-launch-floating-terminal-with-presentation "
        .. shell_escape("omyvoid-plymouth-reset"),
    },
  }
